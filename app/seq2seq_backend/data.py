...
'''
def get_id2line():
    lines=open('/home/james/Downloads/cornell movie-dialogs corpus/movie_lines.txt').read().split('\n')
    id2line = {}
    for line in lines:
        _line = (unicode(line, errors='ignore')).split(' +++$+++ ')
        if len(_line) == 5:
...
def get_conversations():
    conv_lines = open('/home/james/Downloads/cornell movie-dialogs corpus/movie_conversations.txt').read().split('\n')
    convs = [ ]
    for line in conv_lines[:-1]:
        _line = unicode(line, errors='ignore').split(' +++$+++ ')[-1][1:-1].replace("'","").replace(" ","")
...
def process_line(line, dict):
    ...
    wlist = [w.strip() for w in line.split(' ') if w]
    q_indices = pad_seq(wlist, dict, limit['maxq'])
    return np.array(q_indices)
